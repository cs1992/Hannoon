...
jindo.$Agent.prototype.navigator = function() {
	...
	b.chrome = !b.edge && b.webkit && !b.opera && g("Chrome", n) || g("CriOS", n);
	b.safari = !b.edge && b.webkit && !b.chrome && !b.opera && g("Apple", m);
	b.firefox = g("Firefox", n);
	b.mozilla = !b.edge && g("Gecko", n) && !b.safari && !b.chrome && !b.firefox && !b.ie;
	b.camino = g("Camino", m);
	b.netscape = g("Netscape", n);
	b.omniweb = g("OmniWeb", n);
	b.icab = g("iCab", m);
	b.konqueror = g("KDE", m);
	b.mobile = !b.edge && (g("Mobile", n) || g("Android", n) || g("Nokia", n) || g("webOS", n) || g("Opera Mini", n) || g("Opera Mobile", n) || g("BlackBerry", n) || (g("Windows", n) && g("PPC", n)) || g("Smartphone", n) || g("IEMobile", n)) && !(g("iPad", n) || g("Tablet", n));
	b.msafari = !b.edge && ((!g("IEMobile", n) && g("Mobile", n)) || (g("iPad", n) && g("Safari", n))) && !b.chrome && !b.opera && !b.firefox;
...
jindo.$Ajax = function(c, e) {
		...
		if (this._checkCORSUrl(this._url)) {
			if (g && "withCredentials" in g) {
				...
				if (window.XDomainRequest) {
					this._bXDomainRequest = true;return new XDomainRequest()
				}
			}
		} else {
			if (g) {
	...
	case "post":
		...
		this._request = b.call(this);this._checkCORS(this._url, d.type);
		break;case "flash":
...
jindo.$Ajax.prototype._checkCORSUrl = function(a) {
	return /^http/.test(a) && !new RegExp("^http://" + window.location.host, "i").test(a)
};
...
jindo.$Ajax.prototype._onload = (function(a) {
	if (a) {
		return function() {
			...
			var c = this._request.readyState == 4 && (b == 200 || b == 0) || (this._bXDomainRequest && !!this._request.responseText);
			var d;
...
jindo.$Ajax.prototype.request = function(c) {
	...
	if (l == "XHR" && g == "GET" && /MSIE|Trident/.test(window.navigator.userAgent)) {
		m.setRequestHeader && m.setRequestHeader("If-Modified-Since", "Thu, 1 Jan 1970 00:00:00 GMT")
	}
	if ((l == "XHR" || l == "IFRAME" || (l == "FLASH" && d.sendheader)) && m.setRequestHeader) {
	...
	if (m.addEventListener && !(n.indexOf("Opera") > -1) && !(n.indexOf("MSIE") > -1)) {
		if (this._loadFunc) {
			m.removeEventListener("load", this._loadFunc, false)
		}
		this._loadFunc = function(a) {
			clearTimeout(e);
			e = undefined;u._onload(a)
		};m.addEventListener("load", this._loadFunc, false);m.addEventListener("error", this._loadFunc, false)
	} else {
		if (typeof m.onload != "undefined") {
			...
			if (this._bXDomainRequest) {
				...
				m.onload = function(a) {
					...
					u._request.status = 200;s(a)
				};
				m.onerror = function(a) {
					u._request.readyState = 4;
					u._request.status = 404;s(a)
				}
			}
		} else {
			if (window.navigator.userAgent.match(/(?:MSIE) ([0-9.]+)/)[1] == 6 && d.async) {
				var q = function(a) {
					if (m.readyState == 4 && !u._is_abort) {
						...
						u._onload(a);clearInterval(u._interval);
	...
	this._test_url = b;m.send(h);return this
};
...
jindo.$H.prototype.length = function() {
	...
	for (var a in this._table) {
		if (this._table.hasOwnProperty(a)) {
			if (typeof Object.prototype[a] != "undefined" && Object.prototype[a] === this._table[a]) {
				continue
			}
			b++
		}
...
jindo.$Element.prototype.appear = function(b, c) {
	...
	if (a.hasTransition) {
		jindo.$Element.prototype.appear = function(g, h) {
			...
			h = h || function() {};
			var f = function() {
				h();d.show();this.removeEventListener(a.name + "End", arguments.callee, false)
			};
			var e = this._element;
			if (!this.visible()) {
